...
function removeLikedVideoFromList(userThatLiked, idOfVideo, titleOfVideo, lengthOfVideo, uploaderOfVideo, uploaderPicOfVideo){
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "services/liked_video_remove.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            loadAllLikedVideos();
            console.log("Video eltavolitva a kedveltekbol!");
            document.getElementById("likedvideostatus-modal").innerHTML = `
            <div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content">
                <div class="modal-body" id="likedvideostatus-body">
                    <img src="pageelements/warning_icon.png" width="70" height="auto"/><br><br>
                    <h4>Videó eltávolítva a kedveltekből!</h4>
                </div>
                </div>
            </div>
            </div>`;
...
function copyURLShare(idOfVideoToShare){
  navigator.clipboard.writeText("https://vidflow.barnatech.hu/watch?id="+idOfVideoToShare);
